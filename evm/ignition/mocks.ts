...
;(async () => {
  ...
  const Uniswap = await ethers.deployContract('MockUniswapV3Factory', signers[0])
  ...
  for (const currency of currencies) {
    ...
    if (rate) {
      const intrate = Math.round((1 / rate) * 10 ** 8)
  ...
  const params = {
    ...
    uniswapUniversalRouter: '0x3A9D48AB9751398BbFa63ad67599Bb04e4BdF98b', // sepolia
    weth: Tokens['WETH'].target,
