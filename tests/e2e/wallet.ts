 ...
 */
import { createE2EProvider, e2eConnector, setChain as _setChain, setSigningAccount } from '@wonderland/walletless'
...
export const connector = () => e2eConnector({
  provider: provider,
  account: accounts[0][0], // default: first hardhat account
  debug: true,
})
...
window.setAccount = (id: number) => {
  return setSigningAccount(provider, accounts[id][0])
}
